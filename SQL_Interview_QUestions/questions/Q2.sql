...
with cte as( 
select student_name, attempt_year, marks, lag(marks) over(partition by student_name order by attempt_year)
as previous_marks from student_marks_report)

select student_name,attempt_year as current_year, marks as current_marks, previous_marks, ((marks-previous_marks)*100/previous_marks) as YOY_performance from cte order by student_name,attempt_year;
